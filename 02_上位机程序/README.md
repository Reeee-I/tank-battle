...
| 文件 | 用途 |
|---|---|
| `main.py` | 程序入口：参数解析 / 创建 SerialHub 与 Game / 主循环（窗口 1280x720 宽屏） |
| `game.py` | Game 类：主循环 / 碰撞 / 道具 / 渲染 / 开始与结算界面 / 地图系统（逻辑战场仍为 800x600） |
| `assets.py` | 素材加载与缓存（坦克/心形/边框/地面/标题/障碍）；缺素材自动回退矢量画法 |
...
| `requirements.txt` | Python 依赖清单（仍仅 pygame、pyserial，无新增） |
| `tests/` | 自动化自测（无硬件可跑，见下） |
| `tools/serial_probe.py` | 串口裸监听工具：实时打印解析出的玩家包，烧录后先跑它验证链路 |
| `素材/` | UI 换肤素材（坦克蓝/红、心形、边框、地面、Tank Battle 标题、障碍等） |

## 界面(UI 换肤, 4:3)

- **窗口 4:3(800×600)** = 逻辑战场 1:1；**背景封面裁剪**(不拉伸)、**边框拉伸**(不裁剪)。
- 蓝/红**像素坦克**(加大)、**蓝/红彗尾弹**(同尺寸；增强弹=更大+白热核心+粗色环)、**心形生命**(蓝/红/灰, 适中)、灰色金属方块障碍(**整块等比缩放、不裁切**)。
- 顶部保留**边框自带的 TANK BATTLE 金板**；左右上角**正方形坦克头像**(PLAYER 名称/心形不与边框重叠)；底部**比分上移居中** + FPS。
- **缺素材自动回退**到原有矢量画法(换机/缺文件也能跑)。
