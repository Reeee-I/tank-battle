 ...
 * ========================  【音效测试版】  ==================================
 *
 * 本文件由 485手柄/main_handle.c 派生，仅叠加"蜂鸣器音效"，其余代码与原版逐行一致。
 * 角色：485 手柄板（玩家手边，被 485 中转板点名应答）→ SOUND_ENABLED = 1
 *
 ...
 *   ④ K1（开火）按下瞬间 → Sound_Play(SOUND_FIRE)；
 *   ⑤ 方向键按住期间每 100ms → Sound_Play(SOUND_MOVE)；
 *   ⑥ 中转板转发的下行血量帧（AA D1/D2 HP，仅本板命令码命中）血量下降沿 →
 *      SOUND_DAMAGE，血量归零 → SOUND_GAMEOVER（与 LED 血条逻辑并行）。
 *   注：本文件为可选补充（仓库主推 485 三板方案的两块手柄共用此源），
 *      由工程 Define PLAYER_ID=1/2 区分玩家，与有线 USB 版音效逻辑完全一致。
 *
...
void myUart2Rxd_callback(void)
{
...
#endif
    {
        ...
        /* 【音效·测试新增】下行血量联动发声（规则与有线 USB 版一致）：
         * 血量下降沿 → SOUND_DAMAGE；血量归零 → SOUND_GAMEOVER；
         * 首帧/回血/重发相同血量均不发声。 */
