 ...
 * ========================  【音效测试版】  ==================================
 *
 * 本文件由 MCU/main.c 派生，仅叠加"蜂鸣器音效"，其余代码与原版逐行一致。
 * 角色：有线手柄（板载 USB / 串口直连电脑，玩家手边）→ SOUND_ENABLED = 1
 *
 ...
 *   ⑥ 收到 PC 下行血量帧（AA D1/D2 HP）时：血量下降沿 → SOUND_DAMAGE，
 *      血量归零 → SOUND_GAMEOVER（与原有 LED 血条逻辑并行，互不影响）。
 *   编译：PLAYER_ID 宏仍照旧由 Keil 工程 C51 Define 设置（1=玩家1 / 2=玩家2）。
 *
...
void myUart1Rxd_callback(void)
{
    ...
    if ((lastHp != 0xFF) && (dlRxBuf[2] < lastHp))
    {
        if (dlRxBuf[2] == 0)
        {
